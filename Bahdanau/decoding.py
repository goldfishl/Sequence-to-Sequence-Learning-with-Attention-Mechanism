...
class TeacherDecoder:
    ...
    def __call__(self, decoder, encoder_outputs, hidden_state):
        ...
        for i in range(self.max_len):
            out,  hidden_state, atten_weights = decoder(
                input_token, hidden_state, encoder_outputs, self.mask)
            outputs[:, i, :] = out
...
class GreedyDecoder:
    ...
    def __call__(self, decoder, encoder_outputs, hidden_state):
        ...
        if self.use_stop:
            sentence_idx = []
            for i in range(self.max_len):
                out, hidden_state, atten_weight = decoder(input_token, hidden_state,
...
class BeamSearchNode:
    def __init__(self, idx, logp, prev_node, hidden_state, atten_weights):
        self.idx = idx
...
class BeamSearchDecoder:
    ...
    def __call__(self, decoder, encoder_outputs, hidden_state):
        ...
        out, hidden_state, atten_weights = decoder(input_token, hidden_state,
        ...
        for idx, logp in zip(topk_idx, topk_logp):
            best_nodes.append(BeamSearchNode(idx, logp, None, hidden_state, atten_weights))
        while len(end_nodes) < self.width:
            temp_nodes = []
            dec_step += 1
            for i, node in enumerate(best_nodes):
                out, hidden_state, atten_weights = decoder(node.idx.reshape(-1),
                                                           node.hidden_state,
                                                           encoder_outputs, mask)
                out = out.squeeze().log_softmax(0)
                topk_logp, topk_idx = torch.topk(out, self.width - len(end_nodes))
                for idx, logp in zip(topk_idx, topk_logp):
                    temp_nodes.append(BeamSearchNode(idx, logp, node, hidden_state, atten_weights))
