...
import spacy
from Luong.decoding import GreedyDecoder, BeamSearchDecoder
...
def evaluate(model, val_iterator,
    ...
    with torch.no_grad():
        ...
        for batch in val_iterator:
            ...
            src_len = src_len.to("cpu")  # pack_pad function require len variable on cpu
...
def single_decode(model, src, dataset, decode_method, beam_width=0, reverse_src=True):
    """
...
# def single_decode(model, src, dataset, decode_method, beam_width, revers_src=True):
#     """
#     single sentence test for English to German translation
#     """
#     nlp = spacy.load('en_core_web_md')
#     tokens = [token.text.lower() for token in nlp.tokenizer(src.strip())]
#     index = dataset.srcField.vocab.lookup_indices(tokens)
#     bos_idx = dataset.srcField.vocab.__getitem__(dataset.special_tokens['bos'])
#     eos_idx = dataset.srcField.vocab.__getitem__(dataset.special_tokens['eos'])
#     if revers_src:
#         index = index[::-1]
...
#     if decode_method == "greedy":
#         greedy = GreedyDecoder(bos_idx)
#         sentences_idx, atten_weights = model(src, src_len, greedy)
#         for sentence_idx in sentences_idx:
#             translation = [dataset.trgField.vocab.itos[i] for i in sentence_idx]
#     return translation, atten_weights
